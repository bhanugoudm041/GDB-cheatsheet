...
*(gdb) break main+64       set a breakpoint on a function+address
*(gdb) break *0x08048590       set a breakpoint on at an address
...
#Printing addresses inside registers
*(gdb) print/x $eip        	print one register
*(gdb) print $eip        	print one register(detailed view)
*(gdb) print/x &main     print address of function

#Examine data inside memory
*(gdb) help x        	show formats for x
*(gdb) x/4b &gd1         print variable gd1 in 4bytes format
*(gdb) x/x &gx        	print variable gx in hex format
*(gdb) x/4wx &main       print 4 longs at start of \fImain\fR in hex
*(gdb) x/gf &gd1         print variable gd1 in giant or 8bytes format
*(gdb) x/4bx 0x0804858b        print 4bytes at speicific address

#setting values
*(gdb)set $eip=0x0804858b  setting register value to a specific address
*(gdb)set *0x08048588=0x44  setting a address value bytes to specific values

#jumping to specific address or function
*(gdb)jump *0x0804858b  jumping to address
*(gdb)jump *main+69  jumping to function+address
```
...
### Breakpoints - break or b is used
```
*(gdb) break main       set a breakpoint on a function
*(gdb) break main+64       set a breakpoint on a function+address
*(gdb) break *0x08048590       set a breakpoint on at an address
